<template>
  <div class="colors">
    <router-link
      ...
      :key="index"
      :to="`/product-details/${color.productId}`"
      @click="clickButton(index, color.productId)"
    > 
      <svg
        class="colors__svg"
        width="37"
...
export default {
  ...
  computed: {
    getCurrentIndex() {
      this.activeIndex = this.colors.findIndex(
        (color) => color.color.id === this.choosenColor.id
      )
    },
  },
  methods: {
    clickButton(index, sendId) {
      this.activeIndex = index
      // this.$emit(['getOtherColorProductId', sendId])
    },
